...
class BaseBallFragment : Fragment() {
    ...
    override fun onResume() {
        ...
        storeViewModel.getStore(lat, lng,"스크린야구장", FIRST_CALL)
    }
    ...
    private fun initRecyclerView() {
        baseball_list.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!recyclerView.canScrollVertically((1)) && lastVisibleItemPosition >= 0) {
                    storeAdapter?.getLastStoreId(lastVisibleItemPosition)?.let { storeViewModel.getStore(lat,lng, "스크린야구장", it) }
                }
            }
        })
    }
